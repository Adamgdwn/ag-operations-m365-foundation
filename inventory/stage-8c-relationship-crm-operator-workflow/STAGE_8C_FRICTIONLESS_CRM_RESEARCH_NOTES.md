...
- relaxes `ReceivedDate` and `ItemOwner` so they do not block manual intake;
- marks source/system fields as read-only in the form formatter.

Read-back verification passed on 2026-06-17 with zero bad counts, including the
new intake-experience checks.
