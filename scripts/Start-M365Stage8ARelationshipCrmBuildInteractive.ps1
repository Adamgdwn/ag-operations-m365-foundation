...
# Opens a visible PowerShell window for the Stage 8A Relationship CRM builder.
# The target script is dry-run by default and requires typed approval when
# -Apply is supplied.
...
function Start-VisiblePowerShellConsole {
    ...
    $command = @(
        ...
        "echo This creates only approved Stage 8A CRM Lists, page, views, and navigation when -Apply and typed approval are used.",
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8ARelationshipCrmBuild.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8A Relationship CRM build..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8A CRM Build" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
