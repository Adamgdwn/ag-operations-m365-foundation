...
$skus = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "subscribed-skus.json"))
$users = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "users.json"))
$roleMembers = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "directory-role-members.json"))
$oauth2Grants = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "oauth2-permission-grants.json"))
...
$conditionalAccess = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "conditional-access-policies.json"))
$authMethods = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "user-authentication-method-summary.json"))
$sharePointSites = @(Import-JsonIfExists (Join-Path $resolvedInventoryPath "sharepoint-sites.json"))
...
$lines.Add(("| SharePoint site sharing read | {0} |" -f ($(if (@($sharePointSites).Count -gt 0) { "yes" } else { "no" }))))
$lines.Add("")
...
if (@($broadGrants).Count -eq 0) {
    ...
    $lines.Add("| Matched scope | Consent type | Client ID | Principal ID |")
    $lines.Add("|---|---|---|---|")
    foreach ($grant in $broadGrants | Sort-Object matched, clientId) {
        $lines.Add(("| {0} | {1} | {2} | {3} |" -f $grant.matched, $grant.consentType, $grant.clientId, $grant.principalId))
    }
}
$lines.Add("")
...
$lines.Add("")
$lines.Add("1. Decide Security Defaults now versus Business Premium / Conditional Access path.")
...
$lines.Add("5. Record any accepted risks in the Decision Register once Stage 6 Planner/Teams is live.")
$lines.Add("")
