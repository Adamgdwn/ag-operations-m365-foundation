...
# Opens a visible PowerShell window for the Stage 8 workspace backing structure
# builder. The target script is dry-run by default and requires typed approval
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8WorkspaceBackingBuild.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8 workspace backing structure build..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8 Workspace Backing Build" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
