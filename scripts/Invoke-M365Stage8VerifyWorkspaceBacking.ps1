...
# Stage 8 - read-only backing structure verification.
# Verifies expected routing pages, Lists, fields, views, libraries, folders, and
# navigation links. It performs no tenant writes.
...
$transcriptPath = Join-Path $resolvedOutputRoot ("stage-8-workspace-backing-verify-{0}.log" -f $timestamp)
$pageCsvPath = Join-Path $resolvedOutputRoot ("stage-8-backing-pages-{0}.csv" -f $timestamp)
$listCsvPath = Join-Path $resolvedOutputRoot ("stage-8-backing-lists-{0}.csv" -f $timestamp)
$libraryCsvPath = Join-Path $resolvedOutputRoot ("stage-8-backing-libraries-{0}.csv" -f $timestamp)
$navCsvPath = Join-Path $resolvedOutputRoot ("stage-8-backing-navigation-{0}.csv" -f $timestamp)
$summaryPath = Join-Path $resolvedOutputRoot "STAGE_8_WORKSPACE_BACKING_VERIFY.md"
...
Write-Host "Microsoft 365 Stage 8 - Workspace Backing Structure Verification" -ForegroundColor Cyan
...
if (-not (Get-Module -ListAvailable -Name PnP.PowerShell)) {
    throw "PnP.PowerShell is not available in this PowerShell host. Re-run through scripts\Start-M365Stage8VerifyWorkspaceBackingInteractive.ps1, which prefers pwsh.exe."
}
...
$libraryResults = foreach ($library in $config.libraries) {
    $existingLibrary = Get-PnPList -Identity ([string]$library.title) -Includes RootFolder -ErrorAction SilentlyContinue
    if ($null -eq $existingLibrary) {
        [pscustomobject]@{
            Title = [string]$library.title
            Exists = $false
            MissingFolders = (($library.folders | ForEach-Object { [string]$_ }) -join "; ")
            Status = "Missing"
        }
        continue
    }

    $rootFolder = [string]$existingLibrary.RootFolder.ServerRelativeUrl
    $missingFolders = @()
    foreach ($folder in $library.folders) {
        if (-not (Test-Stage8FolderExists -BaseUrl $rootFolder -Child ([string]$folder))) {
            $missingFolders += [string]$folder
        }
    }
    ...
    [pscustomobject]@{
        Title = [string]$library.title
        Exists = $true
        MissingFolders = ($missingFolders -join "; ")
        Status = if ($missingFolders.Count -eq 0) { "Present" } else { "Partial" }
    }
}
...
$libraryResults | Export-Csv -LiteralPath $libraryCsvPath -NoTypeInformation -Encoding UTF8
...
$partialLibraries = @($libraryResults | Where-Object { $_.Status -ne "Present" })
$missingNav = @($navResults | Where-Object { $_.Status -ne "Present" })
$result = if ($missingPages.Count -eq 0 -and $partialLists.Count -eq 0 -and $partialLibraries.Count -eq 0 -and $missingNav.Count -eq 0) { "PASS" } else { "PARTIAL" }

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add("# Stage 8 Workspace Backing Structure Verification")
$lines.Add("")
...
$lines.Add(("Library CSV: {0}" -f $libraryCsvPath))
...
$lines.Add("## Libraries")
$lines.Add("")
$lines.Add("| Status | Library | Missing folders |")
$lines.Add("|---|---|---|")
foreach ($libraryResult in $libraryResults) {
    $lines.Add(("| {0} | {1} | {2} |" -f $libraryResult.Status, $libraryResult.Title, $libraryResult.MissingFolders))
}
$lines.Add("")
...
Write-Host ("Stage 8 workspace backing verification {0}: {1}" -f $result, $summaryPath) -ForegroundColor $resultColor
