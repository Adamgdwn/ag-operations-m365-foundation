...
# Stage 8 - approval-gated SharePoint backing structure builder.
# Dry-run by default. With -Apply and typed approval, it creates missing routing
# pages, Microsoft Lists, document libraries, folders, and concrete Quick Launch
# links. It does not change permissions, invite guests, widen external sharing,
# grant apps, publish Forms links, delete anything, or overwrite existing pages.

$ErrorActionPreference = "Stop"

$workspaceRoot = Split-Path -Parent (Split-Path -Parent $PSCommandPath)
$transcriptRoot = Join-Path $workspaceRoot "inventory\stage-8-client-workspace-reference\workspace-backing-structure"
New-Item -ItemType Directory -Path $transcriptRoot -Force | Out-Null
$transcriptPath = Join-Path $transcriptRoot ("stage-8-workspace-backing-build-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))
...
$sections
<p><em>Generated from the Stage 8 workspace backing structure config. This page is a routing surface; Lists, libraries, Planner, and decision records remain the durable sources of truth.</em></p>
"@
}
...
function Join-ServerRelativeUrl {
    ...
    $encoded = [System.Uri]::EscapeDataString($Child).Replace("%2F", "/")
    return "$($BaseUrl.TrimEnd('/'))/$encoded"
}

function Test-Stage8FolderExists {
    param(
        [string]$BaseUrl,
        [string]$Child
    )

    $rawUrl = "$($BaseUrl.TrimEnd('/'))/$Child"
    $encodedUrl = Join-ServerRelativeUrl -BaseUrl $BaseUrl -Child $Child
    $folder = Get-PnPFolder -Url $rawUrl -ErrorAction SilentlyContinue
    if ($null -ne $folder) {
        return $true
    }

    $folder = Get-PnPFolder -Url $encodedUrl -ErrorAction SilentlyContinue
...
function Resolve-NavigationUrl {
    ...
    switch ($kind) {
        ...
        "Library" {
            return Get-LibraryUrl -Title $value
        }
        "LibraryFolder" {
            $parts = $value -split "/", 2
            if ($parts.Count -ne 2) {
                throw "LibraryFolder targets must use 'Library/Folder': $value"
            }
            return Join-ServerRelativeUrl -BaseUrl (Get-LibraryUrl -Title $parts[0]) -Child $parts[1]
        }
        default {
...
Write-Host "Microsoft 365 Stage 8 - Workspace Backing Structure Build" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Planned routing pages:" -ForegroundColor Cyan
foreach ($page in $config.pages) {
    Write-Host ("- {0} ({1})" -f $page.title, $page.fileName) -ForegroundColor White
}
Write-Host ""
Write-Host "Planned Lists:" -ForegroundColor Cyan
foreach ($list in $config.lists) {
    Write-Host ("- {0}" -f $list.title) -ForegroundColor White
}
Write-Host ""
Write-Host "Planned libraries/folders:" -ForegroundColor Cyan
foreach ($library in $config.libraries) {
    Write-Host ("- {0}: {1}" -f $library.title, (($library.folders | ForEach-Object { [string]$_ }) -join ", ")) -ForegroundColor White
}
Write-Host ""
...
if (-not $Apply) {
    Write-Host "Dry run only. Re-run with -Apply and type the approval phrase to create missing backing structure." -ForegroundColor Green
    try {
...
if (-not (Get-Module -ListAvailable -Name PnP.PowerShell)) {
    throw "PnP.PowerShell is not available in this PowerShell host. Re-run through scripts\Start-M365Stage8WorkspaceBackingBuildInteractive.ps1, which prefers pwsh.exe."
}
Import-Module PnP.PowerShell -ErrorAction Stop

$approvalPhrase = [string]$config.approvalPhrase
$approval = Read-Host "Type '$approvalPhrase' to create missing Stage 8 backing structure"
...
foreach ($list in $config.lists) {
    ...
    foreach ($column in $list.columns) {
        Add-Stage8Field -ListTitle ([string]$list.title) -Column $column
    }

    foreach ($view in $list.views) {
        Add-Stage8View -ListTitle ([string]$list.title) -View $view
    }
}
...
foreach ($library in $config.libraries) {
    ...
    $existingLibrary = Get-PnPList -Identity ([string]$library.title) -ErrorAction SilentlyContinue
    if ($null -eq $existingLibrary) {
        New-PnPList -Title ([string]$library.title) -Template DocumentLibrary -OnQuickLaunch:$false | Out-Null
        Write-Host ("  [OK] Library created: {0}" -f $library.title) -ForegroundColor Green
    }
    else {
        Write-Host ("  [skip] Library exists: {0}" -f $library.title) -ForegroundColor Gray
    }

    if (-not [string]::IsNullOrWhiteSpace([string]$library.description)) {
        Set-PnPList -Identity ([string]$library.title) -Description ([string]$library.description) | Out-Null
    }

    $libraryInfo = Get-PnPList -Identity ([string]$library.title) -Includes RootFolder
    $rootFolder = [string]$libraryInfo.RootFolder.ServerRelativeUrl
    foreach ($folder in $library.folders) {
        if (Test-Stage8FolderExists -BaseUrl $rootFolder -Child ([string]$folder)) {
            Write-Host ("  [skip] Folder exists: {0}" -f $folder) -ForegroundColor Gray
            continue
        }

        Add-PnPFolder -Name ([string]$folder) -Folder $rootFolder | Out-Null
        Write-Host ("  [OK] Folder created: {0}" -f $folder) -ForegroundColor Green
    }
}
...
Write-Host ""
Write-Host "Stage 8 workspace backing structure build complete. Run read-only verification next." -ForegroundColor Green
