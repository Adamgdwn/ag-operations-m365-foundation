...
# Opens a visible PowerShell window for the read-only Stage 8C CRM workflow verifier.
...
function Start-VisiblePowerShellConsole {
    ...
    $command = @(
        ...
        "echo This is a read-only verification of Stage 8C CRM workflow lists, fields, views, page, and navigation.",
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8CVerifyRelationshipCrmOperatorWorkflow.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8C CRM workflow verification..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8C CRM Workflow Verify" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
