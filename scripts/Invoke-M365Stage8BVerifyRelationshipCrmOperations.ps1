...
# Stage 8B - read-only Relationship CRM operations verification.
...
$transcriptPath = Join-Path $resolvedOutputRoot ("stage-8b-crm-operations-verify-{0}.log" -f $timestamp)
$fieldCsvPath = Join-Path $resolvedOutputRoot ("stage-8b-crm-operations-fields-{0}.csv" -f $timestamp)
$lookupCsvPath = Join-Path $resolvedOutputRoot ("stage-8b-crm-operations-lookups-{0}.csv" -f $timestamp)
$viewCsvPath = Join-Path $resolvedOutputRoot ("stage-8b-crm-operations-views-{0}.csv" -f $timestamp)
$pageCsvPath = Join-Path $resolvedOutputRoot ("stage-8b-crm-operations-pages-{0}.csv" -f $timestamp)
$navCsvPath = Join-Path $resolvedOutputRoot ("stage-8b-crm-operations-navigation-{0}.csv" -f $timestamp)
$summaryPath = Join-Path $resolvedOutputRoot "STAGE_8B_RELATIONSHIP_CRM_OPERATIONS_VERIFY.md"
...
Write-Host "Microsoft 365 Stage 8B - Relationship CRM Operations Verification" -ForegroundColor Cyan
...
if (-not (Get-Module -ListAvailable -Name PnP.PowerShell)) {
    throw "PnP.PowerShell is not available in this PowerShell host. Re-run through scripts\Start-M365Stage8BVerifyRelationshipCrmOperationsInteractive.ps1."
}
...
$lookupResults = foreach ($lookup in $config.lookupFields) {
    $field = Get-PnPField -List ([string]$lookup.list) -Identity ([string]$lookup.internalName) -Includes TypeAsString,Indexed -ErrorAction SilentlyContinue
    [pscustomobject]@{
        List = [string]$lookup.list
...
$lines.Add("# Stage 8B Relationship CRM Operations Verification")
$lines.Add("")
...
$lines.Add(("| Operational fields | {0} |" -f $badFields.Count))
$lines.Add(("| Operational views | {0} |" -f $badViews.Count))
...
Write-Host ("Stage 8B Relationship CRM operations verification {0}: {1}" -f $result, $summaryPath) -ForegroundColor $resultColor
