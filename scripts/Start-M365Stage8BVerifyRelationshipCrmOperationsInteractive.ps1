...
# Opens a visible PowerShell window for read-only Stage 8B CRM operations
# verification.
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8BVerifyRelationshipCrmOperations.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8B CRM operations verification..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8B CRM Verify" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
