...
# Opens a visible PowerShell window for recording the approved Stage 7
# governance decision into the existing Decision Register and Agent Action Log.
...
function Start-VisiblePowerShellConsole {
    ...
    $command = @(
        ...
        "echo This writes only to Decision Register and Agent Action Log.",
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage7RecordGovernanceDecision.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 7 governance decision record..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 7 Decision Record" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
