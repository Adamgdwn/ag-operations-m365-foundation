...
# Opens a visible PowerShell window for the Stage 8 workspace shape builder.
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8WorkspaceShapeBuild.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8 workspace shape build..." -ForegroundColor Cyan
...
if ($IncludePlaceholderNavigation) {
    $arguments += "-IncludePlaceholderNavigation"
}

Start-VisiblePowerShellConsole -Title "M365 Stage 8 Workspace Shape Build" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
