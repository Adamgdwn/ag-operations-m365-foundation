...
# Opens a visible PowerShell window for read-only Stage 8 workspace backing
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8VerifyWorkspaceBacking.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8 workspace backing structure verification..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8 Backing Verify" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
