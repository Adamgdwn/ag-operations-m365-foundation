...
# Opens a visible PowerShell window for read-only Stage 8A Relationship CRM
# verification.
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8AVerifyRelationshipCrm.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8A Relationship CRM verification..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8A CRM Verify" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
