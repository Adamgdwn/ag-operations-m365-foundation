...
# Opens a visible PowerShell window for the read-only Stage 8 workspace shape
# verification.
...
$targetScript = Join-Path $scriptRoot "Invoke-M365Stage8VerifyWorkspaceShape.ps1"
...
Write-Host "Opening visible PowerShell window for Stage 8 workspace shape verification..." -ForegroundColor Cyan
...
Start-VisiblePowerShellConsole -Title "M365 Stage 8 Workspace Shape Verify" -PowerShellPath $powerShellHost.Source -Arguments $arguments -WorkingDirectory $workspaceRoot
