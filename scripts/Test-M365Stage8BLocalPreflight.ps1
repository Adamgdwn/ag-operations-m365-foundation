param(
    ...
    [string]$OutputPath = ".\inventory\stage-8b-relationship-crm-operations\STAGE_8B_LOCAL_PREFLIGHT.md"
)

# Stage 8B - local-only Relationship CRM operations preflight.
...
if (Test-Path -LiteralPath $resolvedConfigPath) {
    try {
        ...
        Add-Result -Results $results -Check "Stage 8B CRM operations config parses as JSON" -Passed $true -Detail $resolvedConfigPath
    }
    catch {
        Add-Result -Results $results -Check "Stage 8B CRM operations config parses as JSON" -Passed $false -Detail $_.Exception.Message
        $config = $null
    }
}
else {
    Add-Result -Results $results -Check "Stage 8B CRM operations config exists" -Passed $false -Detail $resolvedConfigPath
    $config = $null
}

if ($null -ne $config) {
    Add-Result -Results $results -Check "Config is Stage 8B" -Passed ([string]$config.stage -eq "8B") -Detail ("Stage: {0}" -f $config.stage)
    ...
    Add-Result -Results $results -Check "Config has lookup fields" -Passed (@($config.lookupFields).Count -ge 8) -Detail ("Lookups: {0}" -f @($config.lookupFields).Count)
    Add-Result -Results $results -Check "Config has six CRM list operation blocks" -Passed (@($config.lists).Count -eq 6) -Detail ("Lists: {0}" -f @($config.lists).Count)
    Add-Result -Results $results -Check "Config has operational views" -Passed ((@($config.lists | ForEach-Object { @($_.views).Count }) | Measure-Object -Sum).Sum -ge 10) -Detail "views defined"
    Add-Result -Results $results -Check "Config has approval phrase" -Passed ([string]$config.approvalPhrase -eq "apply-stage-8b-crm-operations") -Detail ([string]$config.approvalPhrase)
}
...
Add-Result -Results $results -Check "PnP.PowerShell module available" -Passed ($null -ne $pnp) -Detail ($(if ($null -ne $pnp) { ("{0} {1}" -f $pnp.Name, $pnp.Version) } else { "required for live CRM operations apply" }))

$pnpCommands = @("Connect-PnPOnline", "Get-PnPList", "Get-PnPField", "Add-PnPField", "Add-PnPFieldFromXml", "Set-PnPField", "Get-PnPView", "Add-PnPView", "Set-PnPView", "Add-PnPPage", "Add-PnPPageSection", "Add-PnPPageTextPart", "Set-PnPPage", "Get-PnPNavigationNode", "Add-PnPNavigationNode")
...
$lines.Add("# Stage 8B Relationship CRM Operations Local Preflight")
$lines.Add("")
...
$lines.Add("")
$lines.Add("1. Run `.\scripts\New-M365Stage8BRelationshipCrmOperationsPacket.ps1`.")
$lines.Add("2. Run `.\scripts\Invoke-M365Stage8BRelationshipCrmOperationalize.ps1` for a dry run.")
$lines.Add("3. Run `.\scripts\Start-M365Stage8BRelationshipCrmOperationalizeInteractive.ps1 -Apply` after approval and type `apply-stage-8b-crm-operations`.")
$lines.Add("4. Run `.\scripts\Start-M365Stage8BVerifyRelationshipCrmOperationsInteractive.ps1` for read-only verification.")
$lines.Add("")

Set-Content -LiteralPath $resolvedOutputPath -Value $lines -Encoding UTF8

if ($failed.Count -eq 0) {
    Write-Host "Stage 8B local preflight PASS: $resolvedOutputPath" -ForegroundColor Green
}
else {
    Write-Host ("Stage 8B local preflight FAIL ({0} issue(s)): {1}" -f $failed.Count, $resolvedOutputPath) -ForegroundColor Red
    exit 1
}
