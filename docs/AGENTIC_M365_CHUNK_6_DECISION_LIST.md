...
| A6-13 | When does QUO enter the agent lane? | B10a local readiness is complete; B10b closes an implementation-ready placeholder/design pack; B10c/later live proof comes after exact approval while volume is low. | Approved number(s), event class, ingress option, signature/secret plan, retention, duplicate rule, disable path, and outbound block. |
