...
Website-side fallback / CTA cleanup has been handled on the Labs site. Any tenant-side
shutdown of the old Forms flow is reversible owner/admin maintenance, not an open site
gate. Verification scripts (scratchpad): submit-engage-test.js (drives the live form via
warm-Edge/CDP) + verify-engage-record.js.
...
serves both brands (payload `source` = `Guided AI Journey`). Website-side fallback / CTA
cleanup has been handled on the Journey site. Any tenant-side shutdown of the old Forms
flow is reversible owner/admin maintenance, not an open site gate. Verification scripts
(scratchpad): submit-journey-test.js + verify-journey-record.js.
...
**Legacy Forms path (superseded on the websites; retained here for provenance):**
```
...
4. Both brands passed. Website-side CTA / fallback cleanup is handled on both sites.
   Tenant-side old Forms-flow shutdown remains optional, reversible owner/admin
   maintenance (`scripts/flow-builder/set-flow-state.js`); do not delete forms or
   disable flows without explicit scope and approval.
...
- Reversible: the custom HTTP flow can be turned OFF instantly; any old Forms-flow
  retirement should remain reversible during transition.
